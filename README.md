...
- THE ACTION DISPATCHED BY `createAsyncThunk()` the promise lifecycle STATES
###           *** PENDING, FULFILLED, REJECTED ***
#### In createAsyncThunk PASS the ACTION TYPE 'resource/actionType' 
#### `createAsyncThunk('resourceType/actionType', ASYNC)`--> The dispatched ACTION TYPES:
...
#### - REFRESH NOTE* `const creates_A_Slice_Of_Store = createSlice( { accepts an OPTION argument} )` </br>
...
#### - SLICES RESPOND TO ACTION CREATORS of type: `slicesName.actions.actionType`
#####  --> ACTION CREATORS that userSlice responds to ==> usersSlice.actions.addUser 
